...
import { TenantPermissionsListAsync,DeleteTenantPermissionAsync } from '../../Apis/TenantPermissions.js';

export default function TenantPermissionsList() {
  const [TenantPermissionsList, setTenantPermissionsList] = useState([]);
  const navigate = useNavigate();

  async function DeletePermission(id) {
      
    const reslt =await DeleteTenantPermissionAsync(id);
    if (reslt.success) {
      setTenantPermissionsList(prevList => prevList.filter(item => item.id !== id));
      console.log("Deleted Permission with ID:", id);
    }
      else {
      console.error("Failed to delete permission:", reslt.message || "Unknown error");
  ...
  useEffect(() => {
    ...
    const response = await TenantPermissionsListAsync();
    if (response.success){
      setTenantPermissionsList(response.data);
      console.log("Fetched Tenant Permissions:", response.data);
    }else {
      console.error("Failed to fetch Tenant Permissions:", response.message||"Unknown error");
      setTenantPermissionsList([]);
    }
  }
  ...
  const dynamicColumns = TenantPermissionsList.length > 0 ? Object.keys(TenantPermissionsList[0]) : [];
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white border border-slate-200/60 rounded-2xl shadow-sm p-4 overflow-hidden">
        {TenantPermissionsList?.length > 0 ? (
          ...
          <p className="text-center py-8 text-slate-400 font-medium text-sm">No systemic Tenant Permissions payloads found.</p>
        )}
      </div>
    </div>
  );
